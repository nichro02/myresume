...
<<<<<<< HEAD
  const exportPDF = async () => {
    ...
    for (const rawLine of lines) {
      ...
      const isHeader = trimmed.length > 2 && /^[A-Z][A-Z\s&/]+$/.test(trimmed);

      if (isHeader) {
        doc.setFont("helvetica", "bold");
        doc.setFontSize(10);
        doc.setTextColor(29, 158, 117);
        if (y > marginTop + 20) y += 6;
        const wrapped = doc.splitTextToSize(trimmed, maxLineWidth);
        for (const wl of wrapped) {
          if (y + 14 > pageHeight - 40) { doc.addPage(); y = marginTop; }
        ...
        for (const wl of wrapped) {
          if (y + 13 > pageHeight - 40) { doc.addPage(); y = marginTop; }
          doc.text(wl, marginLeft, y);
          y += 13;
        }
      }
